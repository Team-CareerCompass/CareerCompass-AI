...
**정본은 [`docs/AI_CONTRACT_v0.2.md`](docs/AI_CONTRACT_v0.2.md) 다.** BE 가 무엇을 보내고 무엇을 받는지, 그리고 왜 그렇게 정했는지가 거기 있다. 프로바이더로 무엇이 나가는지는 [`docs/PII.md`](docs/PII.md), 건당 얼마인지는 [`docs/COST.md`](docs/COST.md).
...
| 지표 | 규칙 전용 | 규칙 + LLM |
| --- | --- | --- |
| 마감일 (26건) | **26/26**, 날조 0 | **같음** (규칙 값이 우선) |
| 유형 (24건) · 공고 판별 (28건) | 100% | **같음** |
| 문항 (26건) | 23/26 — 사람인 「문항 1」 라벨형은 못 뽑음 | **25/26** — LLM 폴백이 채움 |
| 키워드 (정답 160개) | 재현율 0% | **재현율 51% · 정밀도 66%** · 금지어 3 |
| 우대 조건 (정답 60개) | 재현율 2% | **재현율 47% · 정밀도 74%** |
| LLM 호출 | 0 | 26건 (5건은 규칙이 먼저 걸러 0원) |
| 토큰 · 비용 | — | 40,316 토큰 · **12.1원** (공고당 0.46원) |
| 지연 p50 | — | 2.0초 |
...
**프롬프트 다섯 버전 × 모델 둘을 같은 평가셋으로 쟀다** — [`app/prompts/README.md`](app/prompts/README.md). 결론: DASH-002 는 프롬프트로 더 못 올리고(재현율 45~60% 안에서 자리만 바뀜), HCX-005 는 5배 비용에 우대 조건에서만 낫다. 표본 10건이라 5%p 안쪽은 잡음이다. **v1 + DASH-002 를 쓴다.**
...
| **A4 문서 분류** | — | **BE 가 구현함** |
| **A5 추천·로드맵** | — | **BE 가 구현함** |
| **A6 운영·품질** | 비용 측정·가드·배포 | #28 완료(`docs/COST.md`) · #29 규칙 가드 붙음 · #31 BE 인프라 뒤 |

### 다음에 할 것

1. **#7 키워드 품질** — 라벨·채점기·버전 비교까지 됐다. 남은 것은 **표본 30건**. 그 전에는 프롬프트를 더 만지지 않는다.
...
### 검증에 걸리면 안전한 답을 낸다 (#29)

모델 출력은 **규칙**으로 검증하고, 못 통과하면 이상한 답 대신 미리 정한 안전한 답을 낸다. LLM 으로 LLM 을 검증하지 않는다.

| 엔드포인트 | 검증 (전부 규칙) | 통과 못 하면 |
| --- | --- | --- |
| 파싱 | 전처리에서 지시문 문단 제거 · 마감일 +18개월 상한 · 문항의 민감정보 요구 제거 · 키워드 3개 미만·40자 초과 · JSON 스키마(1회 재요청) | `200 + parsingFailed` — 규칙이 뽑은 것도 안 낸다 |
| 코멘트 | 근거 토큰을 지목하지 않은 문장 제거 · 출력 PII 스크럽 | `null` — BE 는 점수만 낸다 |
| 초안 | 수치·영문 토큰이 경험 요약·공고 제목·질문에 있는지 대조(`keywords` 는 근거 아님) · 글자 수 실측 · 출력 PII 스크럽 | ① 1회 재요청 → ② 걸린 문장 제거 → ③ **입력 문자열만으로 만든 안전 초안** (`factCheck.fallback: true`) |

주입 픽스처 015 가 규칙(마감일 2099)과 모델(type·피싱 문항) 둘 다 뚫는 것을 보고 만든 것들이다. 아직 없는 것: 유해·차별 표현 필터(실물 사례 없음), 한국어 고유명사 날조 탐지(형태 변화 때문에 문자열 대조로 안 됨).
