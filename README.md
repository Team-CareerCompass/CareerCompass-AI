...
**규칙 기반 파이프라인과 평가셋이 돈다.** 엔드포인트 셋은 아직 더미 응답을 내고, 모델은 부르지 않는다.

### 측정 결과 — 규칙 전용 (2026-09-13, 픽스처 12건)
...
| | 범위 | 상태 |
| --- | --- | --- |
| **A0 기반** | 골격·게이트웨이·프롬프트 관리·평가셋·PII | #1 완료 · **#5 러너 섬** · #2 #3 #6 #35 남음 |
| **A1 공고 파싱** | 구조화 파싱·마감일·양식 인식·이미지 공고 | **규칙 부분 완료** · LLM 부분은 자격증명 대기 |
...
1. **#2 게이트웨이 + #3 프롬프트 외부화 + #35 리플레이 캐시** — 한 덩어리다. 따로 하면 캐시 키를 두 번 고친다. **AWS 자격증명이 있어야 실호출로 넘어간다.**
2. **#7 파싱의 LLM 부분** — 규칙이 못 하는 것만 맡는다. 핵심 키워드 의미적 선별 · 유형 판별의 애매한 경우 · 뭉친 텍스트의 문항 추출 · `NOT_A_POSTING` 최종 판정.
3. **픽스처 보강** — 우대 사항 헤딩이 있는 공고 · 「붙임 파일 참조」만 있는 공고. 지금 12건.
4. **#6 PII 문서** — BE 가 `PrivacyMasker` 로 마스킹해 보내므로 범위가 줄었다. 이쪽에서 **프로바이더로 나가는 것**만 정하면 된다.
