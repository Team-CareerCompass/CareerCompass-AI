...
### 측정 결과 — 규칙 전용 (2026-09-12, 픽스처 7건)

| 지표 | 결과 |
| --- | --- |
| 마감일 정확도 | **7/7 (100%)** — 틀림 0 · 못읽음 0 · **날조 0** |
| 유형 판별 | **7/7 (100%)** |
| 문항 개수 | 7/7 — 단, 정답이 전부 「0개」라 **추출 능력은 아직 미검증** |

마감일은 여섯 가지 실제 표현과 함정 셋을 통과한다 — 범위 표현에서 앞 날짜를 집지 않고, 바로 다음 줄의 「결과 발표일」과 운영일정표의 날짜를 마감일로 오인하지 않고, 「지원기간: 선발시부터 6개학기」 같은 지급 기간에 속지 않는다. 연도가 본문에 없으면 추정하되 `yearInferred` 로 표시한다.
...
3. **픽스처 3건 보강** — 채용 텍스트 1 · 대외활동 1 · **`NOT_A_POSTING` 1**. 마지막이 없으면 학사공지 필터가 도는지 확인할 방법이 없다.
