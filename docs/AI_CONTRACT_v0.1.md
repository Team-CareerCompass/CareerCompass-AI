...
**파일이 아니라 텍스트를 받는다.** PDF·DOCX 텍스트 추출과 스캔 PDF OCR 은 이 서비스 안에서 로컬 라이브러리로 하되(보고서 4.4.1), 그 경로는 별도 엔드포인트 `POST /v1/extract-text`(multipart) 로 분리한다. BE 가 S3 에서 파일을 받아 넘긴다.
...
| 이미지 공고 멀티모달 경로 (§1.5) | 신규 이슈 필요 | 공공기관 채용·장학 공지 상당수 |
| 첨부 HWP·PDF 를 열어볼 것인가 | 신규 이슈 필요 | 「붙임 파일 참조」 공고 |
