...
**상한은 엔드포인트 전체에 걸린다** (`CC_REQUEST_DEADLINE_S`, 기본 16초). `CC_LLM_TIMEOUT_S`(18초)는 HTTP 요청 하나당이라, 재요청이 붙으면 합계를 아무도 안 보는 구멍이 있었다 — 파싱은 스키마 재요청으로 최악 두 배, 초안은 톤·단축·사실 재요청이 다 걸리면 6~8회다. 이제 **재요청을 시작하기 전에 남은 시간을 보고, 모자라면 재요청을 포기하고 지금 가진 답으로 끝낸다.** 답이 조금 나쁜 것이 BE 타임아웃에 걸려 통째로 버려지는 것보다 낫다.

포기해도 **규칙으로 지키는 것은 그대로다** — 글자 수 상한은 문장 경계 절단으로, 사실검증은 문장 제거·안전 초안으로. 포기하는 것은 「모델에게 한 번 더 물어보는 것」뿐이다. 건너뛴 재요청은 경고 로그로 남는다.

실측(09-16, `docs/COST.md`): DASH-002 파싱 p50 1.8초·최대 5.1초, 초안 재요청 둘까지 8.0초. **HCX-005 파싱은 최대 15.5초** — 005 로 올리면 재요청 한 번에 20초를 넘으므로 이 시한이 전제다.
...
- **차별 요구는 세 목록 어디에도 담지 않는다** (#29). 공고에 있어도 `keywords`·`preferences`·`formQuestions` 에서 뺀다 — 채용절차법 제4조의3(구직자 본인의 용모·키·체중 등 신체조건, 출신지역·혼인 여부·재산, 직계존비속·형제자매의 학력·직업·재산)과 남녀고용평등법 제7조(모집·채용의 성별 지정)에 해당하는 것. 그대로 올리면 BE 적합도 계산과 코멘트를 타고 사용자에게 「미혼 요건을 충족하지 못합니다」로 도착한다. **공고가 위법인지 판정하지는 않는다** — 전달 경로만 끊는다. 병역·연령은 병역법·청년고용촉진특별법에 근거가 있어 대상이 아니다. 실물 공고 31건에서 제거 0건, 합성 위법 공고에서는 전부 제거(`fixtures/postings/033.md`). BE 에 알림: BE #67
...
- **문장마다 근거를 지목해야 남는다.** 근거를 지목하지 않는 문장은 버리고, 남는 문장이 없으면 그 항목은 `null` 이다. 생성물의 수치·영문 토큰도 근거와 대조해(`factCheck` 와 같은 규칙) 걸리면 그 문장을 뺀다 — 초안과 달리 **재요청하지 않는다.** 코멘트는 `null` 이 허용되고 점수는 그대로 나가기 때문이다
- **근거가 낱말 하나일 때 역량을 단정한 항목은 버린다** — 근거 「설계」로 「설계 능력이 있습니다」를 쓰면 사용자는 해본 적 없는 일을 읽는다. BE 의 우대 매칭이 토큰 부분일치라 이런 근거가 실제로 온다
...
- `factCheck.unverified` — 생성물에 나왔는데 입력(`experienceSummaries` · `postingTitle` · `question`)에서 확인되지 않은 수치·영문 토큰, 그리고 **입력이 한 번도 말하지 않은 자격·수상·소속 표현**(「자격증」「입상」「입사」 등, #29). **LLM 이 아니라 문자열 대조 규칙으로 판정한다.** `keywords` 는 근거가 아니다 — 공고의 요구 기술을 「해 본 경험」으로 쓰면 날조다.
...
- **v0.2 (09-25 보완)** — 스키마는 그대로고 의미가 넓어진 것 셋: `factCheck.unverified` 에 한국어 자격·수상·소속 표현 추가 · 파싱 세 목록에서 차별 요구 제외 · 경험 요약이 비면 모델을 부르지 않고 안전 초안(`usage.totalTokens: 0`). 응답 시간 상한이 엔드포인트 전체에 걸린다(`CC_REQUEST_DEADLINE_S` 16초). BE 에 알렸다 — BE #67
